...
window.addEventListener('mousedown', (e) => { if(e.target === canvas) jump(); });
...
function drawPlayer() {
    ...
    const rotation = Math.min(Math.max(player.velocity * 0.08, -0.6), 0.9);
    ctx.rotate(rotation);

    // ASAS NEON GIGANTES
    const wingFlap = Math.sin(frameCount * 0.25) * 15;
    
    // Asa de trás
    ctx.shadowBlur = 15; ctx.shadowColor = player.color;
    ctx.strokeStyle = player.color; ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.ellipse(-10, -wingFlap, 25, 12, -0.2, 0, Math.PI * 2);
    ctx.stroke();

    // Corpo (Círculo Brilhante)
    ...
    // Asa da frente
    ctx.beginPath();
    ctx.ellipse(-5, wingFlap, 22, 10, 0.2, 0, Math.PI * 2);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.moveTo(15, -2); ctx.lineTo(30, 0); ctx.lineTo(15, 2);
    ctx.fill();
