...
const player = { x: 150, y: 100, velocity: 0, size: 40, color: '#00f2ff' };
...
window.addEventListener('mousedown', (e) => { if(isPlaying) jump(); });
...
function drawPlayer() {
    ...
    const rotation = Math.min(Math.max(player.velocity * 0.1, -0.6), 0.9);
    ctx.rotate(rotation);

    // ASAS NEON TRIANGULARES (SUPER VISÍVEIS)
    const flap = Math.sin(frameCount * 0.3) * 20;
    ...
    ctx.beginPath();
    ctx.moveTo(-5, 0);
    ctx.lineTo(-25, -15 + flap);
    ctx.lineTo(-15, -30 + flap);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.moveTo(15, -5); ctx.lineTo(35, 0); ctx.lineTo(15, 5);
    ctx.fill();
...
function updateShop() {
    ...
    skins.forEach(s => {
        ...
        skinListContainer.innerHTML += `
            <div class="skin-item">
                <div class="skin-preview" style="background:${s.color}; border: 2px solid #fff; border-radius: 50%;"></div>
                <span style="font-weight:bold">${s.name}</span>
                <span class="skin-price">${owned ? 'OK' : s.price + ' $'}</span>
                <button onclick="buySkin('${s.id}')">${active ? 'ATIVO' : (owned ? 'USAR' : 'COMPRAR')}</button>
            </div>`;
    });
}
